...
SkeletonClusterer::SkeletonClusterer(size_t K, std::chrono::time_point<std::chrono::high_resolution_clock> bigbang, const size_t * const center_indices_init_, size_t ndata_in, std::string init_method, double max_time, double min_mE, size_t * const indices_final, size_t * const labels, size_t nthreads, size_t max_rounds, std::string energy, bool with_tests, size_t random_sd, const EnergyInitialiser & energy_initialiser):
  ...
  center_IDs = indices_final;
    ...
    void SkeletonClusterer::signal_cluster_change(size_t k){
      cluster_has_changed[k] = true;
    }
    
    size_t SkeletonClusterer::draw_j_uniform(size_t k){
    ...
    void SkeletonClusterer::default_initialise_with_kmeanspp(){
      ...
      if (km_is_exhausted == true || n_bins == 1){
        ...
        triangular_kmeanspp();//km_is_exhausted);
        if (km_is_exhausted){
    ...
    void SkeletonClusterer::triangular_kmeanspp_aq2(size_t n_bins){
      ...
      auto update_nearest_info = [&a_distance, this](size_t bin, size_t k_start, size_t k_end, size_t i0, size_t i1)
      {
        for (size_t i = i0; i < i1; ++i){
          for (size_t k = k_start; k < k_end; ++k){
            if (kmoo_cc[aq2p_p2buns[bin].k_1(i)*K + k] < aq2p_p2buns[bin].d_1(i) + aq2p_p2buns[bin].d_2(i)){
      ...
      for (size_t bin = 0; bin < n_bins; ++bin){
        ...
        triangular_kmeanspp_after_initial(bin, k0, k1, false);//, km_is_exhausted); //&p2buns_dt[bin]
      ...
      for (size_t bin = 0; bin < n_bins; ++bin){
        ...
        if (nthreads == 1 || multithread_kmpp == false){
          ...
          std::vector<std::thread> threads;
          for (size_t ti = 0; ti < get_nthreads(); ++ti){
            threads.emplace_back([this, ti, bin, k1, non_tail_k, & update_nearest_info] () {update_nearest_info(
              bin, k1, non_tail_k, 
              get_start(ti, get_nthreads(), 0, aq2p_p2buns[bin].get_ndata()),
              get_end(ti, get_nthreads(), 0, aq2p_p2buns[bin].get_ndata()));});
          }
    ...
    void SkeletonClusterer::triangular_kmeanspp(){
     ...
     size_t SkeletonClusterer::get_nthreads(){
      return nthreads;
    }
    
     double SkeletonClusterer::get_nthreads_fl(){
      return nthreads_fl;
    }
     ...
     * get centers [k0, k1).
     * km_is_exhausted reports whether the every sample has a center a distance 0 (when samples duplicated). 
     *  */

 
    

    void SkeletonClusterer::triangular_kmeanspp_after_initial(size_t aq2p_bin, size_t k0, size_t k1, bool from_full_data){
      ...
      if (from_full_data == true) {
        ...
        set_distance_kk = [this](size_t k, size_t kp){
          set_sampleID_sampleID_distance_nothreshold(center_indices_init[k], center_indices_init[kp], kmoo_cc[k*K + kp]);  
        };
      ...
      for (size_t k = k0; k < k1; ++k){
        ...
        if (v_cum_nearest_energies.back() != 0){
          ...
          if (try_uniform_when_exhausted == false){
            ...
            while (std::find(center_indices_init, center_indices_init+k, new_index) != center_indices_init+k){
              new_index = dis(gen)%(K);
            }
    ...
    void SkeletonClusterer::test_parameters_to_tkai(size_t k0, size_t k1, size_t ndata_1, size_t ndata_2){
      if (k0 >= K || k1 < k0 || k1 > K){
    ...
    void SkeletonClusterer::set_center_sample_distance_nothreshold(size_t k, size_t k1, size_t j1, double & distance){
    ...
    void SkeletonClusterer::set_center_center_distance_nothreshold(size_t k1, size_t k2, double & adistance) {
...
void SkeletonClusterer::populate_afk_mc2(){
  ...
  for (size_t i = 0; i < ndata; ++i){
    //metric.set_distance(ptr_datain->at_for_metric(index0), ptr_datain->at_for_metric(i), std::numeric_limits<double>::max(), adistance);
  ...
  for (size_t k = 1; k < K; ++k){
    ...
    for (size_t kp = 0; kp < k; ++kp){
      //metric.set_distance(ptr_datain->at_for_metric(sample_index_current), 
                          //ptr_datain->at_for_metric(center_indices_init[kp]), 
                          //std::numeric_limits<double>::max(), 
                          //adistance);
      
      set_sampleID_sampleID_distance_nothreshold(sample_index_current, center_indices_init[kp], adistance);                    
    ...
    for (size_t m = 1; m < chain_length; ++m){
      ...
      for (size_t kp = 0; kp < k; ++kp){
//        metric.set_distance(ptr_datain->at_for_metric(sample_index_proposal), 
                            //ptr_datain->at_for_metric(center_indices_init[kp]), 
                            //std::numeric_limits<double>::max(), 
                            //adistance);

        set_sampleID_sampleID_distance_nothreshold(sample_index_proposal, center_indices_init[kp], adistance);                                                
   ...
   /* set energy of cluster k, and set as custom cluster statistics */
    ...
    size_t SkeletonClusterer::get_a1(size_t k, size_t j) const{
      return nearest_1_infos[k][j].a_x;
    }
    
     double SkeletonClusterer::get_d1(size_t k, size_t j){
      return nearest_1_infos[k][j].d_x;
    }
    
     double SkeletonClusterer::get_e1(size_t k, size_t j){
      return nearest_1_infos[k][j].e_x;
    }
    
     double SkeletonClusterer::get_e1_tail(size_t k){
      return nearest_1_infos[k].back().e_x;
    }
  
      double SkeletonClusterer::get_E_total(){
      return E_total;
    }
    
     double SkeletonClusterer::get_cluster_energy(size_t k){
...
(For a description of column statistics, consider function get_output_verbose_string())
   ...
   void SkeletonClusterer::pll_put_samples_in_cluster(size_t i_a, size_t i_z, std::vector<size_t> & non_center_IDs){
    ...
    void SkeletonClusterer::final_push_into_cluster_post_kmeanspp(size_t i, size_t k1, size_t k2, double d1, double d2){
     ...
     void SkeletonClusterer::final_push_into_cluster_basic(size_t i, size_t nearest_center, double min_distance){
    ...
    //void triangular_kmeanspp_aq2(size_t * const centers, double * const cc, P2Bundle & p2bun, TData & c_dt, size_t n_bins, bool & km_is_exhausted){
    ...
    size_t SkeletonClusterer::draw_k_uniform(){
      return dis(gen)%K;
    }
    ...
    void SkeletonClusterer::reset_sample_infos(size_t k, size_t j){
     ...
     void SkeletonClusterer::final_push_into_cluster(size_t i, size_t nearest_center, double min_distance, const double * const distances){
  ...
  in_refinement = true;
    ...
    void SkeletonClusterer::initialise_center_indices()  {
      ...
      /* initialisation uniformly according to Bachem et al. */
        ...
        populate_afk_mc2();//initialisation_method, center_indices_init, *ptr_datain, metric, K, ndata, mowri, gen, dis, f_energy);
      }
