...
size_t extract_INT(std::string initialisation_method, size_t prefix_length)
{
  ...
  if (initialisation_method.size() < prefix_length + 1)
  {
    ...
    errm_ss << "invalid initialisation_method " << initialisation_method
            << ". It is not of the form `prefix'-INT, where INT is positive.";
    throw zentas::zentas_error(errm_ss.str());
  }

  std::string digit_substring =
    initialisation_method.substr(prefix_length, initialisation_method.size() - prefix_length);
  ...
  while (striter != digit_substring.end())
  {
    ...
    {
      std::stringstream errm_ss;
      errm_ss << "Unexpected character while attempting to extract integer from "
              << initialisation_method << " (" << digit_substring << ")"
              << ", `" << x << "'";
      throw zentas::zentas_error(errm_ss.str());
    }
    ++striter;
  ...
  for (size_t k = 0; k < K; ++k)
  {
    ...
    if (center_indices_init_in[k] >= ndata)
    {
      throw zentas::zentas_error(
        "initialising center index out of bounds in BaseClusterer constructor");
    }
    for (unsigned j = 0; j < k; ++j)
    {
      ...
      {
        throw zentas::zentas_error(
          "initialising center index at j and k are the same in BaseClusterer constructor");
      }
    }
...
void populate_uniformly(size_t* const                          center_indices_init,
                        size_t                                 K,
                        size_t                                 ndata,
