...
void ExponionData::specific_custom_remove_last(size_t k){
  v_b[k].pop_back();
}
...
void ExponionData::specific_initialise_from_n1n2(const XNInfos &  nearest_1_infos, const XNInfos &  nearest_2_infos){
...
void ExponionData::specific_swap(size_t k1, size_t k2){
...
void YinyangData::specific_custom_remove_last(size_t k){
  glt_ID[k].pop_back();  
}
...
void YinyangData::specific_initialise_from_n1n2(const XNInfos &  nearest_1_infos, const XNInfos &  nearest_2_infos){
  (void)nearest_1_infos;
...
void YinyangData::specific_set_n_groups(size_t K, size_t ndata){
  ...
  rf_n_groups = std::min<size_t>(K/9,  static_cast<size_t>(max_ngbs*1e9 / ((sizeof(size_t) + sizeof(double))*ndata)));
}
...
void RefinementData::initialise_from_n1n2(const XNInfos &  nearest_1_infos, const XNInfos &  nearest_2_infos){
  lower_2.resize(K);
  upper_1.resize(K);
  for (size_t k = 0; k < K; ++k){
...
void SkeletonClusterer::rf_update_sample_info_exponion(){
  ...
  for (size_t k = 0; k < K; ++k){
    ...
    for (size_t kp = prd->cum_in_group[prd->groups[k]]; kp < prd->cum_in_group[prd->groups[k]+1]; ++kp){
      if (kp != k){
    ...
    for (size_t g = 0; g < prd->rf_n_groups; ++g){
      if ((g != prd->groups[k]) && (prd->l_gC[k*prd->rf_n_groups + g] < 2*prd->u1_C[k] + min_cc)){ // do not need to add s to make max_delta_possible_second correct.
        groups_in_vicinity.push_back(g);
        prd->l_gC[k*prd->rf_n_groups + g] = std::numeric_limits<double>::max();
        for (size_t kp = prd->cum_in_group[g]; kp < prd->cum_in_group[g+1]; ++kp){
          set_rf_center_center_distance(k, kp, std::numeric_limits<double>::max(), cc_all[kp]);
          min_cc = std::min(min_cc, cc_all[kp]);
          prd->l_gC[k*prd->rf_n_groups + g] = std::min(prd->l_gC[k*prd->rf_n_groups + g], cc_all[kp]);
        }
      }
    }
    ...
    for (size_t j = 0; j < get_ndata(k); ++j){
      ...
      prd->lower_2[k][j] -= max_delta_possible_second; // this is better than max_prd->delta_C. but is it correct? Yes, but TODO the name is wrong
  ...
  for (size_t start_g = 0; start_g < prd->rf_n_groups; ++start_g){
    for (size_t k = prd->cum_in_group[start_g]; k < prd->cum_in_group[start_g + 1]; ++k){
      ...
      for (size_t kp = prd->cum_in_group[start_g]; kp <prd->cum_in_group[start_g+1]; ++kp){
        set_rf_center_center_distance(k, kp, std::numeric_limits<double>::max(), std::get<0>(v_dk[kp]));
      ...
      std::sort(
      v_dk.begin() + prd->cum_in_group[start_g], 
        ...
        if (global_l_bound < prd->upper_1[k][j]){
          ...
          if (global_l_bound < prd->upper_1[k][j]){
            ...
            if (rf_bound(ID, start_g) < prd->upper_1[k][j]){
              rf_set_2_smallest_ws(k, j, group_ks[start_g].data() + 1, group_ks[start_g].size() - 1, min_k1[start_g], min_d1[start_g], min_d2[start_g]);               
            ...
            // otherwise the lower bound should remain the same. 
              ...
              prd->l_gps[ID*prd->rf_n_groups + start_g] = std::max(prd->l_gps[ID*prd->rf_n_groups + start_g], min_d2[start_g]);
            }
            
            if (min_k1[min_g_global] != k){
              reset_nearest_info(k, j, min_k1[min_g_global], min_d1[min_g_global], f_energy(min_d1[min_g_global]));
            }
          }
        }
      }
...
void SkeletonClusterer::rf_set_2_smallest_ws(size_t k, size_t j, const size_t * ks, size_t nks, size_t & min_k1, double & min_d1, double & min_d2){
...
void SkeletonClusterer::rf_set_2_smallest_group(size_t k, size_t j, size_t g, size_t & min_k1, double & min_d1, size_t & min_k2, double & min_d2){
...
void SkeletonClusterer::rf_set_2_smallest_group(size_t k, size_t j, size_t g, size_t & min_k1, double & min_d1, double & min_d2){
...
std::vector<size_t> SkeletonClusterer::get_subclustered_centers_labels(size_t sub_K) {
  ...
  double sub_max_time = SkeletonClusterer::time_total/(1000*1000*25.); // spent 1/25th of time so far.
  double sub_min_mE = 0;
  ...
  perform_subclustering(sub_K, sub_indices_init, sub_initialisation_method, sub_algorithm, sub_level, sub_max_proposals, sub_capture_output, sub_output_text, sub_seed, sub_max_time, sub_min_mE, sub_max_itok, sub_v_indices_final.data(), sub_v_labels.data(), sub_nthreads, sub_max_rounds, sub_patient, sub_energy, sub_with_tests, sub_bigbang, sub_do_balance_labels);
...
bool SkeletonClusterer::halt_refinement() {
  ...
  size_t time_in_refine = std::chrono::duration_cast<std::chrono::microseconds>(time_now - refine_start).count();    
...
void SkeletonClusterer::output_halt_refinement_reason(){      
  ...
  if (rf_max_rounds < prd->rf_round){
    mowri << "  [" << n_reasons+1 << "] exceeded rf_max_rounds (" << rf_max_rounds << ") of refinement";
    ++n_reasons;
  }
  if (n_reasons == 0){
