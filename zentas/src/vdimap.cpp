...
template <typename T>
void set_mean_variance(std::vector<T> & mean, std::vector<T> & variance, size_t ndata, size_t dimension, const T * const ptr_datain){ 
...
std::vector<size_t> get_indices_decreasing(size_t dimension, const T * const variance){
  ...
  std::sort(v_i.begin(), v_i.end(), [](std::tuple<T, size_t> a, std::tuple<T, size_t> b){ return std::get<0>(a) > std::get<0>(b);});
  if (std::get<0>(v_i.at(0)) < std::get<0>(v_i.back())){
    throw zentas::zentas_error("logic error : the variance is highest variance dimension is lower than the variance of the lowest variance dimension ");
  }

  /* the data is constant in all dimensions */
  if (std::get<0>(v_i[0]) == 0){
    throw zentas::zentas_error("the data appears to be constant. this can be handled, but it seems sufficiently weird that i'm bailing ");
  }
...
template <typename T>
void set_unimat(size_t neigs, size_t dim, T * const evs, std::uniform_int_distribution<size_t> & dis, std::default_random_engine & gen){
  for (size_t e = 0; e < neigs; ++e){
    for (size_t dp = 0; dp < dim; ++dp){
...
template <typename T>
void power_method(T * const eigenvectors, T * const eigenvector_norms, size_t neigs, size_t dim, size_t n_samples_for_covariance, const T * const cov_samples, size_t n_power_iterations){
...
size_t flops_by_matrix = dim * dim * n_samples_for_covariance + dim * dim * neigs * n_power_iterations;
  ...
  for (size_t e = 0; e < neigs; ++e){
    ...
    if (product_by_samples == true){
      for (size_t si = 0; si < n_samples_for_covariance; ++si){
        ...
        for (size_t dp = 0; dp < dim; ++dp){
          sigma_v[dp] += (1./n_samples_for_covariance)*inner_product*cov_samples[si*dim + dp];
        }
      }
    }
...
void vdimap(std::vector<T> & v_mapped, const T * const ptr_datain, size_t ndata, size_t dimension, size_t seed){
  ...
  if (print_times){
    ...
    std::cout << "Time first set mean variance : " <<  std::to_string(time_mean_var00/1000) << std::endl;
  }
  ...
  for (size_t i = 0; i < n_samples_for_covariance; ++i){
    for (size_t dp = 0; dp < dim_nz; ++dp){
      ...
      cov_samples[i*dim_nz + dp] = ptr_datain[sample_indices[i]*dimension + indices_decreasing[dp]] - mean[indices_decreasing[dp]];
    }
  }
  
  /* we initialise the eigenvectors and eigenvalues (to be obtained by power iteration) to uniform noise */
  ...
  power_method(eigenvectors.data(), eigenvector_norms.data(), neigs, dim_nz, n_samples_for_covariance, cov_samples.data(), n_power_iterations);
  ...
  /* we have the eigenvectors, now set first neigs dimensions of v_mapped, and remove from the remaining dims */
  ...
  for (size_t i = 0; i < ndata; ++i){
    ...
    for (size_t dp = 0; dp < dim_nz; ++dp){
      v_mapped[neigs + dim_m*i + dp] = ptr_datain[i*dimension + indices_decreasing[dp]] - mean[indices_decreasing[dp]];
    }
    ...
    for (unsigned e = 0; e < neigs; ++e){
      ...
      for (unsigned dp = 0; dp < dim_nz; ++dp){
        v_mapped[neigs + (dim_nz + neigs)*i + dp] -= inner_product*eigenvectors[e*dim_nz + dp];
      }
    }
  ...
  if (print_times){
    ...
    auto time_post_residual = std::chrono::duration_cast<std::chrono::microseconds>(t3 - t0).count();
  ...
  if (print_times){
    ...
    auto time_post_final_sort = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t0).count();
    std::cout << "Time post final sort : " <<  std::to_string(time_post_final_sort/1000) << std::endl;
  }
...
template void vdimap<float>(std::vector<float> & v_mapped, const float * const ptr_datain, size_t ndata, size_t dimension, size_t seed);
template void vdimap<double>(std::vector<double> & v_mapped, const double * const ptr_datain, size_t ndata, size_t dimension, size_t seed);
