...
LevenshteinInitializer::LevenshteinInitializer(const size_t dict_size, const double c_indel, const double c_switch, const double * const c_indel_arr, const double * const c_switch_arr, bool normalised): dict_size(dict_size), c_indel(c_indel), c_switch(c_switch), c_indel_arr(c_indel_arr), c_switch_arr(c_switch_arr), normalised(normalised) {
  ...
  if (do_tests && c_switch_arr != nullptr){
    ...
    if (do_tests && normalised == true){
      for (unsigned i = 0; i < dict_size; ++i){
        if (c_indel_arr[i] != c_indel_arr[0]){
          ss << "The normalised Levenshtein `metric' is only a true metric when the indel costs are all the same (see abstract of Yujian et al. 2007). ";
          ss << "However, cost_indel[" << i << "] is not the same as cost_indel[0]";
          ss << " (" << c_indel_arr[i] << ") vs (" << c_indel_arr[0] << "). ";
          ss << "As an example of why this constraint is required, consider c_switch = 10, c_indel(1) = 9, c_indel(2) = 10, ";
          ss << "and the sequences S1 = 12  S2 = 121 S3 = 21. The d(S1, S2) + d(S2, S3) = 36/59 and d(S1, S3) = 36/58 : not a metric!\n\n";
          is_fine = false;
        }
      }
    }
    ...
    if (is_fine == false){
      std::string indel_broken = "the cost_switch/cost_indel matrices do not obey the triangle inequality.  ";
      indel_broken += "Consider `reducing' the `inflated' costs. See Wagner and Fischer, 1974 for details. ";
      throw zentas::zentas_error(indel_broken + ss.str());
    }
  }
}

LevenshteinInitializer::LevenshteinInitializer(const double c_indel, const double c_switch, bool normalised): LevenshteinInitializer(0, c_indel, c_switch, nullptr, nullptr, normalised) {}

LevenshteinInitializer::LevenshteinInitializer(const size_t dict_size, const double * const c_indel_arr, const double * const c_switch_arr, bool normalised): LevenshteinInitializer(dict_size, 0., 0., c_indel_arr, c_switch_arr, normalised){}

LevenshteinInitializer::LevenshteinInitializer():LevenshteinInitializer(0,0.,0.,nullptr, nullptr, false) {}
...
template <typename TSDataIn>
LevenshteinMetric<TSDataIn>::LevenshteinMetric(const TSDataIn & datain, size_t nthreads, const LevenshteinInitializer & li){
