...
void indentify(std::string & source){
  ...
  if (std::string::npos == last_lend){
    throw zentas_error("the kernel up for indentification seems suspicious, it seems like it has no new lines in it :\n"  + source);  
  }
...
std::tuple<std::string, unsigned> splitnumeric(std::string alphanum){
  ...
  if (split_point == std::string::npos){
    throw zentas::zentas_error("This error is being thrown from stringutilbase.cpp, function splitnumeric. It seems like the input string `" + alphanum + "' has no digits in it.");
  }
...
std::string getdirfromfn(const std::string & fn){
  ...
  if (morcels[0].compare("") != 0){
    throw zentas_error("The string passed to getdirfromfn is not a valid path as there is no leading / .");
  }
...
std::string get_padded(unsigned x, unsigned length){
  ...
  for (unsigned sp = 0; sp < n_pads; ++sp){
    padded = padded + ' ';
  }
  return padded;
}
