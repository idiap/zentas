...
void indentify(std::string& source)
{
  ...
  {
    throw zentas_error("the kernel up for indentification seems suspicious, it seems like it has "
                       "no new lines in it :\n" +
                       source);
  }
...
std::tuple<std::string, unsigned> splitnumeric(std::string alphanum)
{
  ...
  {
    throw zentas::zentas_error("This error is being thrown from stringutilbase.cpp, function "
                               "splitnumeric. It seems like the input string `" +
...
std::string getdirfromfn(const std::string& fn)
{
  ...
  {
    throw zentas_error(
      "The string passed to getdirfromfn is not a valid path as there is no leading / .");
  }
