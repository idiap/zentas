      ...
      for (size_t j = 0; j < get_ndata(k); ++j){        
        ...
        if (global_l_bound < prd->upper_1[k][j]){
          ...
          if (global_l_bound < prd->upper_1[k][j]){
            ...
            // if we computed the distance to second nearest in this group, it should be the new lower bound.
            ...
            for (size_t g = 0; g < prd->rf_n_groups; ++g){
              ...
              if (g != global_min_g){ 
                if (min_d1 < prd->upper_1[k][j]){
                  if (g < global_min_g){
                    prd->l_gps[ID*prd->rf_n_groups + global_min_g] = std::min(prd->l_gps[ID*prd->rf_n_groups + global_min_g], prd->upper_1[k][j]);
                  }
                  else{
