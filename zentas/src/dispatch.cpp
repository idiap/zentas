                        ...
                        size_t                   ndata)
{
  ...
  else if (energy_initialiser.get_critical_radius() > 0 && energy.compare("squarepotential") != 0)
  {
    throw zentas::zentas_error(
      "critical radius > 0 is only allowed of with squarepotential energy");
  }
  ...
  {
    std::stringstream errm;
    errm << "Something wrong with algorithm-level combination:    algorithm=" << algorithm
         << "  level=" << level
         << ". Either the algorithm does not exist, or the level of optimisation is not "
            "implemented for the algorithm. Currently the supported algorithms {levels} are: \n";
