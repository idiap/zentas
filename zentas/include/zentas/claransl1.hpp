...
class ClaransL1 : public BaseClarans
{
  ...
  ClaransL1(const SkeletonClustererInitBundle& sb, const ExtrasBundle& eb)
    : BaseClarans(sb, eb),
      up_dists_centers_old_k_to(new double[sb.K]),
      dists_centers_old_k_to(up_dists_centers_old_k_to.get()),
      up_dists_centers_new_k_to(new double[sb.K]),
      dists_centers_new_k_to(up_dists_centers_new_k_to.get())
  {
   ...
   * **********************
   * ********************************************************************************************************/
  virtual void put_sample_custom_in_cluster(size_t              i,
  ...
  // virtual  void reset_sample_custom(size_t k, size_t j, size_t nearest_center, const double *
  // const distances) final override{
  ...
  virtual void custom_info_test() override final { nearest_2_infos_margin_test(); }

  virtual void put_sample_in_cluster(size_t i) override final { base_put_sample_in_cluster(i); }
  ...
  {
    update_center_center_info_l1(
      center_nearest_center, dists_centers_old_k_to, dists_centers_new_k_to);
  }
