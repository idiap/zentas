...
#include <zentas/initialisation.hpp>
#include <zentas/outputwriter.hpp>
#include <zentas/tenergyfunc.hpp>
...
class RefinementData
{
  ...
  // YINYANG SPECIFIC
  std::vector<double>
    l_gps;  // lower bounds on distances to groups. size ndata*rf_n_groups. mapped from glt_ID.
  std::vector<size_t>
                                   t_gps;  // times when lower bounds set. size ndata*rf_n_groups.  mapped from glt_ID.
  ...
  virtual void specific_initialise_from_n1n2(const XNInfos& nearest_1_infos,
                                             const XNInfos& nearest_2_infos) = 0;
...
class ExponionData : public RefinementData
{
  ...
  virtual void specific_initialise_from_n1n2(const XNInfos& nearest_1_infos,
                                             const XNInfos& nearest_2_infos) override final;
...
class P2Bundle
{
  ...
  public:
  size_t& k_1(size_t i) { return k_1_[i]; }

  size_t& k_2(size_t i) { return k_2_[i]; }

  double& d_1(size_t i) { return std::get<0>(d12_[i]); }

  double& d_2(size_t i) { return std::get<1>(d12_[i]); }

  size_t& ori(size_t i) { return ori_[i]; }

  size_t get_ndata() const { return ndata; }
...
class SkeletonClustererInitBundle
{
  ...
  SkeletonClustererInitBundle(size_t                                                      K_,
                              size_t                                                      nd_,
...
class SkeletonClusterer
{
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock> t_update_centers_start,
    t_update_centers_end, t_update_sample_info_end, t_redistribute_end,
    t_update_all_cluster_statistics_end;
  size_t ncalcs_update_centers_start, ncalcs_update_centers_end, ncalcs_update_sample_info_end;
  ...
  * ***************** */
  ...
  virtual void
  set_center_sample_pp_distance(size_t k, size_t bin, size_t i, double& adistance) = 0;
  virtual void set_center_sample_distance(
    size_t k, size_t k1, size_t j1, double threshold, double& distance) = 0;
  virtual void
  set_center_sampleID_distance(size_t k, size_t i, double threshold, double& distance) = 0;
  virtual void
  set_sampleID_sampleID_distance(size_t i1, size_t i2, double threshold, double& distance) = 0;
  virtual void set_sample_sample_distance(
    size_t k1, size_t j1, size_t k2, size_t j2, double threshold, double& adistance) = 0;
  virtual void
  set_center_center_distance(size_t k1, size_t k2, double threshold, double& adistance) = 0;
  ...
  void reset_nearest_info_basic(
    size_t k, size_t j, size_t k_nearest, double d_nearest, double e_nearest);
  void
  reset_nearest_info(size_t k, size_t j, size_t k_nearest, double d_nearest, double e_nearest);
  ...
  void
  triangular_kmeanspp_after_initial(size_t aq2p_bin, size_t k0, size_t k1, bool from_full_data);
  ...
  /* TODO certain variables should be private. */
  protected:
  virtual void put_nearest_2_infos_margin_in_cluster_post_kmeanspp(size_t k1,
                                                                   size_t k2,
                                                                   double d2,
                                                                   double e2) = 0;
  virtual void initialise_with_kmeanspp()                                     = 0;
  virtual void put_sample_custom_in_cluster(size_t, size_t, const double* const) {}
  /* when implementing this, one must guarantee that cluster_has_changed is correctly set in this
   * function */
  ...
  size_t get_a1(size_t k, size_t j) { return nearest_1_infos[k][j].a_x; }

  double get_d1(size_t k, size_t j) { return nearest_1_infos[k][j].d_x; }

  double get_e1(size_t k, size_t j) { return nearest_1_infos[k][j].e_x; }

  double get_e1_tail(size_t k) { return nearest_1_infos[k].back().e_x; }

  double get_E_total() { return E_total; }

  double get_cluster_energy(size_t k) { return cluster_energies[k]; }
  ...
  void final_push_into_cluster(size_t              i,
  ...
  void rf_set_2_smallest_ws(size_t        k,
                            size_t        j,
                            const size_t* ks,
                            size_t        nks,
                            size_t&       min_k1,
  ...
  public:
  /* data */
  virtual void zero_refinement_sum(size_t k)
  {
    (void)k;
  ...
  virtual void append_zero_to_rf_sum_data()
  {
    throw zentas::zentas_error("virtual function append_zero_to_rf_sum_data not possible");
  }
  // virtual void a_to_rf_sum_data(){throw zentas::zentas_error("virtual function
  // append_zero_to_rf_sum_data not possible");  }
  virtual void append_zero_to_old_rf_center_data()
  {
    throw zentas::zentas_error("virtual function append_zero_to_rf_center_data not possible");
  }
  virtual void set_old_rf_center_data(size_t k)
  {
    (void)k;
    throw zentas::zentas_error("virtual function append_zero_to_rf_center_data not possible");
  }
  virtual void set_delta_rf_new_and_old(size_t k, double threshold, double&)
  {
    (void)k;
    (void)threshold;
    throw zentas::zentas_error("virtual function get_delta_rf_new_and_old not possible");
  }
  virtual void set_sum_abs_rf_new_and_old(size_t k, double&)
  {
    (void)k;
    throw zentas::zentas_error("virtual function get_sum_abs_rf_new_and_old not possible");
  }
  virtual void set_rf_center_data(size_t k)
  {
    (void)k;
    throw zentas::zentas_error("virtual function set_rf_center_data not possible");
  }
  virtual std::string string_for_rf_center(size_t k)
  {
    (void)k;
    throw zentas::zentas_error("strinf_for_rf_center not possible");
  }

  /* metric */
  virtual void
  set_rf_center_sample_distance(size_t k, size_t k1, size_t j1, double threshold, double& distance)
  {
    (void)k;
    (void)k1;
    (void)j1;
    (void)threshold;
    (void)distance;
    throw zentas::zentas_error("virtual function set_rf_center_sample_distance not possible");
  }
  virtual void
  set_rf_center_center_distance(size_t k1, size_t k2, double threshold, double& adistance)
  {
    (void)k1;
    (void)k2;
    (void)threshold;
    (void)adistance;
    throw zentas::zentas_error("virtual function set_rf_center_center_distance not possible");
  }
  std::vector<size_t> get_subclustered_centers_labels(
    size_t sub_K);  // {(void)sub_K; throw zentas::zentas_error("cluster_centers not possible"); };
  virtual void rf_cumulative_correction(size_t k_new, size_t k, size_t j)
  {
    (void)k_new, (void)k;
    (void)j;
