...
struct BaseDataIn
{
  ...
  BaseDataIn(const ConstLengthInitBundle<TAtomic>& ib)
    : BaseDataIn(ib.ndata, ib.dimension, ib.data)
  {
  }
  ...
  {
    std::stringstream ss;
    ss << "currently no string function, this is from BaseDataIn. To display sample " << i
       << " requires that the virtual method be specialised";
    return ss.str();
  }
};
...
struct BaseConstLengthDataIn : public BaseDataIn<TAtomic>
{
  ...
  const TAtomic* at_for_metric(size_t i) const
  {
...
{
  public:
  DenseVectorDataUnrootedIn(const ConstLengthInitBundle<TAtomic>& ib)
    : BaseConstLengthDataIn<TAtomic>(ib)
  {
  }
  const TAtomic* at_for_move(size_t i) const
  {
...
{
  public:
  DenseVectorDataRootedIn(const ConstLengthInitBundle<TAtomic>& ib)
    : BaseConstLengthDataIn<TAtomic>(ib)
  {
  }
  size_t at_for_move(size_t i) const { return i; }
...
class BaseVarLengthDataIn : public BaseDataIn<TAtomic>
{
  ...
  BaseVarLengthDataIn(const VariableLengthInitBundle<TAtomic>& ib)
    : BaseVarLengthDataIn(ib.ndata, ib.sizes, ib.data)
  {
  }

  size_t get_size(size_t i) const { return sizes[i]; }

  const TAtomic* get_data(size_t i) const { return BaseDataIn<TAtomic>::data + c_sizes[i]; }
  ...
  void unrooted_viability_test()
  {
    ...
    {
      std::stringstream ss;
      ss << "In unrooted_viability_test, with mean size=" << mean_size
         << " and max size=" << max_size
         << ". The max size is more than 5 times larger than mean size. This means a potentially "
            "large waste of memory, and `cold cache'. Consider the rooted version of this "
            "algorithm. (rooted = true).";
      throw zentas::zentas_error(ss.str());
    }
  }

  size_t get_max_size() const { return max_size; }
  size_t get_mean_size() const { return mean_size; }
};
...
class BaseStringDataIn : public BaseVarLengthDataIn<TAtomic>
{
  ...
  {
    return Sample(BaseVarLengthDataIn<TAtomic>::sizes[i],
...
class StringDataUnrootedIn : public BaseStringDataIn<TAtomic>
{
  ...
  public:
  StringDataUnrootedIn(const VariableLengthInitBundle<TAtomic>& ib) : BaseStringDataIn<TAtomic>(ib)
  {
    BaseVarLengthDataIn<TAtomic>::unrooted_viability_test();
  }
  const VariableLengthSample<TAtomic> at_for_move(size_t i) const
  {
    return VariableLengthSample<TAtomic>(BaseVarLengthDataIn<TAtomic>::sizes[i],
                                         BaseDataIn<TAtomic>::data +
                                           BaseVarLengthDataIn<TAtomic>::c_sizes[i]);
  }
...
class SparseVectorDataInBase : public BaseVarLengthDataIn<TAtomic>
{
  ...
  SparseVectorDataInBase(const SparseVectorDataInitBundle<TAtomic>& ib)
    : BaseVarLengthDataIn<TAtomic>(ib.ndata, ib.sizes, ib.data), indices_s(ib.indices_s)
  {
  }
  ...
  {
    return SparseVectorSample<TAtomic>(BaseVarLengthDataIn<TAtomic>::sizes[i],
                                       BaseDataIn<TAtomic>::data +
                                         BaseVarLengthDataIn<TAtomic>::c_sizes[i],
...
class SparseVectorDataUnrootedIn : public SparseVectorDataInBase<TAtomic>
{
  ...
  SparseVectorDataUnrootedIn(const SparseVectorDataInitBundle<TAtomic>& ib)
    : SparseVectorDataInBase<TAtomic>(ib)
  {
    BaseVarLengthDataIn<TAtomic>::unrooted_viability_test();
  }
...
class SparseVectorDataRootedIn : public SparseVectorDataInBase<TAtomic>
{
  ...
  public:
  SparseVectorDataRootedIn(const SparseVectorDataInitBundle<TAtomic>& ib)
    : SparseVectorDataInBase<TAtomic>(ib)
  {
  }
  size_t at_for_move(size_t i) const { return i; }
