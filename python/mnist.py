...
if (socket.gethostname() == 'idbean'):
  raise RuntimeError("You're on idbean. This means you're gonna need to set some paths in mnist.py - desole (see hardpaths.py)")
  

else:
  #(socket.gethostname() == 'goudurix12'):
  #set the path to infiniminst
  infiexec = os.path.join(hardpaths.infipath, "infimnist")

#else:
  #print "(((Hint : you are  " + socket.gethostname() + ")))"
  #raise RuntimeError("You're not goudurix12, you're not idbean, who the jolly rogers are you?")
  
#set path to data dirs

paths = {
'original' : os.path.join(hardpaths.infipath, 'generateddata/original'), 
'projected' : os.path.join(hardpaths.infipath, 'generateddata/projected'),
'concatenated' : os.path.join(hardpaths.infipath, 'generateddata/concatenated')}
